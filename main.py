...
from utils.auth import login, login_as_guest, is_super_admin
...
if st.session_state.user is None:
    ...
    # 已登录，显示侧边栏导航
    st.sidebar.write(f"欢迎，{st.session_state.user['username']}")
    if st.sidebar.button("退出登录"):
        st.session_state.user = None
        st.rerun()

    # 根据用户角色显示不同的页面选项
    if is_super_admin(st.session_state.user):
        pages = ["资产查询", "部门采购总支出", "资产详情", "新增资产", "编辑资产状态", "账号权限设置"]
    elif st.session_state.user["role"] == "employee":
        pages = ["资产查询", "部门采购总支出", "资产详情"]
    else:  # 访客
        pages = ["资产查询", "资产详情"]
